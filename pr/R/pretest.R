...
sizeDistortionTest <- function(r, x, lags, level = c("0.95", "0.90", "0.80")) {
  ...
  #TODO data loss vs clear lenght
  # Equation (1) of Campbell and Yogo (2005)
  #lm_1 <- lm(x[(lag+2):N] ~ x[(lag+1):(N-1)])
  #smry_1 <- summary(lm_1)
  ...
  table_df_gls <- as.numeric(row.names(pretest_table))
  df_gls_lookup <- table_df_gls[which.min(abs(table_df_gls - c(df_gls)) )]

  pretest_ci <- c(pretest_table[as.character(df_gls_lookup),1][[1]], pretest_table[as.character(df_gls_lookup),2][[1]])

  #test the null hypothesis that the actual size exceeds 7.5%
  reject_null <- ( ci[1]<pretest_ci[1] | ci[2]<pretest_ci[2] )
