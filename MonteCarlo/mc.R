# this file generates mc_output/table1.tex
# change input parameters to generate mc_output/table.tex
...
DEBUG <- FALSE
PLOT <- FALSE
for (N in c(50, 150, 200)){
  N <- N+1 # correct for the loss of 1 observation due to lagging
  for (C in c(0, -1, -10, -15)){
    print(C)
    for (delta in c(-0.99, -0.9)){
...
print(xtable(master_df[ , c("Obs","c","rho","delta","T.test","Bonf.Q.test","Q.test","Bonf.Q.test.two.sided")],
            digits=c(0,0,0,3,2,4,4,4,4), type = "latex"), file = "mc_output/table1.tex")
